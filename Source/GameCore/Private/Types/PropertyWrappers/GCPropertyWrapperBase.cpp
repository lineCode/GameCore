	...
	: FGCPropertyWrapperBase()
{
	...
	Property = FindFProperty<FProperty>(PropertyOwner->GetClass(), InPropertyName); // get the FProperty so we can use push model with it
	...
	// Ensure this property exists on the owner and that it is an InChildScriptStruct!
	{
		...
		const FStructProperty* StructProperty = CastField<FStructProperty>(Property.Get());
		if ((StructProperty && StructProperty->Struct == InChildScriptStruct) == false)
		{
			UE_LOG(LogGCPropertyWrapper, Error, TEXT("%s(): The given FProperty ``%s::%s`` is not a(n) %s!"), ANSI_TO_TCHAR(__FUNCTION__), *(InPropertyOwner->GetClass()->GetName()), *(Property->GetFName().ToString()), *(InChildScriptStruct->GetName()));
			check(0);
		}
	}
